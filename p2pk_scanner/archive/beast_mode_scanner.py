#!/usr/bin/env python3
"""
BEAST MODE P2PK Scanner - High Performance Version
Ultra-optimized multithreaded scanner with batch insertion, write-behind caching,
and database optimizations for maximum throughput.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(Path(__file__).parent.parent / 'logs' / 'beast_mode_scanner.log'),
...
def worker(block_queue: queue.Queue, thread_name: str, db_manager: BeastModeDatabaseManager):
    """Worker thread for processing blocks."""
    ...
    try:
        while True:
            ...
            # Process block with profiling
            try:
                # Profile RPC call
                rpc_start_time = time.time()
                block = bitcoin_rpc.get_block_by_height(height)
                rpc_duration = time.time() - rpc_start_time
                
                with metrics_lock:
                    performance_metrics['rpc_calls'] += 1
                    performance_metrics['rpc_time_total'] += rpc_duration
                    performance_metrics['rpc_time_min'] = min(performance_metrics['rpc_time_min'], rpc_duration)
                    performance_metrics['rpc_time_max'] = max(performance_metrics['rpc_time_max'], rpc_duration)
                ...
                for tx in block.get('tx', []):
                    ...
                    for p2pk_transaction in p2pk_transactions:
                        db_manager.add_transaction(p2pk_transaction)
                        p2pk_count += 1
                        total_found += 1
                ...
                with metrics_lock:
                    ...
                    performance_metrics['transactions_processed'] += tx_count
                    performance_metrics['p2pk_found'] += p2pk_count
...
def keyboard_listener():
    """Listen for keyboard input to stop gracefully."""
...
def report_performance_stats():
    ...
    with stats_lock:
        ...
        logger.info(f"  Total transactions processed: {performance_stats['total_transactions_processed']:,}")
        logger.info(f"  Total addresses found: {performance_stats['total_addresses_found']:,}")
        logger.info(f"  Batch inserts performed: {performance_stats['batch_inserts_performed']:,}")
        logger.info(f"  Queue operations: {performance_stats['queue_operations']:,}")


def report_detailed_performance_metrics():
    """Report detailed performance metrics for profiling."""
    with metrics_lock:
        # Calculate averages
        if performance_metrics['rpc_calls'] > 0:
            performance_metrics['rpc_time_avg'] = performance_metrics['rpc_time_total'] / performance_metrics['rpc_calls']
        
        if performance_metrics['db_operations'] > 0:
            performance_metrics['db_time_avg'] = performance_metrics['db_time_total'] / performance_metrics['db_operations']
        
        if performance_metrics['batch_flushes'] > 0:
            performance_metrics['batch_flush_time_avg'] = performance_metrics['batch_flush_time_total'] / performance_metrics['batch_flushes']
        ...
        print(f"  Success rate: {((performance_metrics['blocks_processed'] / max(performance_metrics['blocks_processed'] + performance_metrics['blocks_failed'], 1)) * 100):.2f}%")
...
def main():
    """Main function for the beast mode scanner."""
    import argparse
    
    parser = argparse.ArgumentParser(description='BEAST MODE P2PK Scanner - High Performance')
    ...
    parser.add_argument('--threads', type=int, default=48, help='Number of threads (default: 48)')
    ...
    logger.info("🚀 Starting BEAST MODE P2PK Scanner...")
