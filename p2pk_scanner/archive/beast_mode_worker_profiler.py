#!/usr/bin/env python3
"""
BEAST MODE Worker Profiler - Profile Worker Thread Performance
Profiles the actual worker threads where RPC calls and database operations happen.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(Path(__file__).parent.parent / 'logs' / 'beast_mode_worker_profiler.log'),
...
worker_profiles = {}
worker_profile_lock = threading.Lock()
...
def profiled_worker(block_queue: queue.Queue, thread_name: str, db_manager: BeastModeDatabaseManager):
    """Worker thread that processes blocks with profiling."""
    global total_blocks_scanned
    
    # Create a profiler for this worker thread
    ...
    try:
        ...
        # Save the profile data for this worker
        with worker_profile_lock:
            worker_profiles[thread_name] = {
                'stats': stats,
...
def report_worker_profiles():
    """Report profiling results from all worker threads."""
    print("\n" + "="*80)
    print("WORKER THREAD PROFILING RESULTS")
    print("="*80)
    
    with worker_profile_lock:
        print(f"Number of worker profiles collected: {len(worker_profiles)}")
        if not worker_profiles:
            print("No worker profiles collected. Worker threads may have finished before profiling data was saved.")
            return
            
        for thread_name, profile_data in worker_profiles.items():
            print(f"\n🧵 {thread_name.upper()} PROFILE:")
            print("-" * 60)
            print(f"Total function calls: {profile_data['total_calls']:,}")
            print(f"Total time: {profile_data['total_time']:.3f}s")
            
            # Get the top 20 functions for this worker
...
def main():
    """Main function for the worker profiler."""
    import argparse
    
    parser = argparse.ArgumentParser(description='BEAST MODE P2PK Scanner - Worker Thread Profiler')
    ...
    logger.info("🚀 Starting BEAST MODE P2PK Scanner with Worker Thread Profiling...")
    ...
    try:
        ...
        for i in range(threads):
            ...
            t = threading.Thread(target=profiled_worker, args=(block_queue, tname, db_manager), daemon=False)
        ...
        # Wait for worker threads to finish and collect profiling data
        logger.info("Waiting for worker threads to finish...")
        for t in worker_threads:
            t.join(timeout=10)
        
        # Generate worker thread profiles
        report_worker_profiles()
